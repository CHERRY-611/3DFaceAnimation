...
class Wav2Vec2ForSpeechClassification(Wav2Vec2PreTrainedModel):
    ...
    def merged_strategy(
        ...
        if mode == "mean":
            ...
            outputs = self.temporal_att(hidden_states)
        else:
            raise Exception(
...
class Wav2Vec2ForCustomSpeechClassification(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.pooling_mode = config.pooling_mode
    ...
    def forward(
        ...
        # hidden_states = outputs[0]
        all_hidden_states = outputs.hidden_states  # Tuple of all L+1 layers
        # 마지막 4개 층 평균
        hidden_states = torch.stack(all_hidden_states[-2:], dim=0).mean(dim=0) 
        hidden_states1 = linear_interpolation(hidden_states, 50, 30, output_len=frame_num)
        hidden_states = self.merged_strategy(hidden_states1, mode=self.pooling_mode)
        logits = self.classifier(hidden_states)
