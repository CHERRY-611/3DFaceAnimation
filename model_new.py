...
class EmoTalk(nn.Module):
    ...
    def forward(self, data):
        ...
        audio = data["input12"]
        print("📦 BEFORE PROCESSOR:", data["input12"].shape)
        # → 리스트로 변환 (batch 처리)
        audio_list = [x.cpu().numpy() for x in audio]  # [B][T]
        ...
        inputs12 = processed.input_values.squeeze(1).to(self.device) 

        print("📦 AFTER PROCESSOR:", inputs12.shape)
        ...
        inputs21 = self.feature_extractor(
            [x.cpu().numpy() for x in data["input21"]],
            sampling_rate=16000,
            return_tensors="pt",
            padding="longest"
        ).input_values.squeeze(1).to(self.device)
                
        # inputs21 = self.feature_extractor((data["input21"]), sampling_rate=16000, padding=True,
        #                                   return_tensors="pt").input_values.to(self.device)
        # inputs12 = self.feature_extractor((data["input12"]), sampling_rate=16000, padding=True,
        #                                   return_tensors="pt").input_values.to(self.device)
        ...
        level_idx = data["level"].cpu().tolist()  # 예: [0, 1]
        onehot_level = torch.from_numpy(self.one_hot_level[level_idx]).to(self.device).float()

        person_idx = data["person"].cpu().tolist()
        onehot_person = torch.from_numpy(self.one_hot_person[person_idx]).to(self.device).float()
      ...
      #  if data["target11"].shape[0] == 1:
        ...
        seq_len11 = hidden_states11.shape[1]
        seq_len12 = hidden_states12.shape[1]
        nhead = 4  # decoder nhead 수

        tgt_mask11 = self.biased_mask1[:1, :seq_len11, :seq_len11].repeat(batch_size * nhead, 1, 1).to(self.device)
        tgt_mask22 = self.biased_mask1[:1, :seq_len12, :seq_len12].repeat(batch_size * nhead, 1, 1).to(self.device)
    ...
    def predict(self, audio, level, person):
        ...
        inputs12 = self.processor((audio), sampling_rate=16000, return_tensors="pt",
        ...
        inputs12 = processed.input_values.to(self.device) 

        # inputs12 = self.feature_extractor((audio), sampling_rate=16000, padding=True,
        #                                   return_tensors="pt").input_values.to(self.device)
